...
U_out2 = zeros(length(recX), length(recY), length(w));
U_out3 = zeros(length(recX), length(recY), length(w));
U_s = zeros(length(recX), length(recY), length(w));

U_inc = zeros(length(w));
U_inc2 = zeros(length(w));
U_inc3 = zeros(length(w));
U_tgt = zeros(length(w));
U_tgt2 = zeros(length(w));
U_tgt3 = zeros(length(w));
...
U_outSpec2 = zeros(length(recX), length(recY), length(wSpec));
U_outSpec3 = zeros(length(recX), length(recY), length(wSpec));
U_sSpec = zeros(length(recX), length(recY), length(wSpec));

U_incSpec = zeros(length(wSpec));
U_incSpec2 = zeros(length(wSpec));
U_incSpec3 = zeros(length(wSpec));
U_tgtSpec = zeros(length(wSpec));
U_tgtSpec2 = zeros(length(wSpec));
U_tgtSpec3 = zeros(length(wSpec));
...
for XX = 1:length(recX)
    for YY = 1:length(recY)
        ...
        % Distance between transmitter and target2
        d_trans_tgt2 = sqrt((transX(1)-tgt2X)^2 + (transY(1)-tgt2Y)^2);
        % Distance between transmitter and target mirror image
        d_trans_tgtMirror2 = sqrt((transX(1)-tgt2X)^2 + (transY(1)+tgt2Y)^2);
        
        % Distance between transmitter and target2
        d_trans_tgt3 = sqrt((transX(1)-tgt3X)^2 + (transY(1)-tgt3Y)^2);
        % Distance between transmitter and target mirror image
        d_trans_tgtMirror3 = sqrt((transX(1)-tgt3X)^2 + (transY(1)+tgt3Y)^2);
        ...
        % Distance between target and receiver
        d_tgt_rec2 = sqrt((tgt2X-recX(XX))^2 + (tgt2Y-recY(YY))^2);
        % Distance between target and receiver mirror image
        d_tgt_recMirror2 = sqrt((tgt2X-recX(XX))^2 + (tgt2Y+recY(YY))^2);
        
        % Distance between target and receiver
        d_tgt_rec3 = sqrt((tgt3X-recX(XX))^2 + (tgt3Y-recY(YY))^2);
        % Distance between target and receiver mirror image
        d_tgt_recMirror3 = sqrt((tgt3X-recX(XX))^2 + (tgt3Y+recY(YY))^2);
        ...
        for WW = 1:length(w)
            ...
            % Incident field transmitter to target2
            U_inc_comps2(1) = greens(d_trans_tgt2, w(WW));
            U_inc_comps2(2) = -1 * greens(d_trans_tgtMirror2, w(WW));
            U_inc2(WW) = sum(U_inc_comps2);
            
            % Incident field transmitter to target3
            U_inc_comps3(1) = greens(d_trans_tgt3, w(WW));
            U_inc_comps3(2) = -1 * greens(d_trans_tgtMirror3, w(WW));
            U_inc3(WW) = sum(U_inc_comps3);
            
            % Total field at target
            U_tgt(WW)=U_inc(WW)/(1-tau*(1/(4*pi))*(exp(i*w(WW)/c*2*tgtY))/(2*tgtY));
            
            % Total field at target2
            U_tgt2(WW)=U_inc2(WW)/(1-tau2*(1/(4*pi))*(exp(i*w(WW)/c*2*tgt2Y))/(2*tgt2Y));
            
            % Total field at target3
            U_tgt3(WW)=U_inc3(WW)/(1-tau3*(1/(4*pi))*(exp(i*w(WW)/c*2*tgt3Y))/(2*tgt3Y));
            ...
            % Field at receiver not influenced by target field or tau
            U_out_comps2(1) = greens(d_tgt_rec2, w(WW));
            U_out_comps2(2) = -1 * greens(d_tgt_recMirror2, w(WW));
            U_out2(XX,YY,WW) = sum(U_out_comps2);
            
            % Field at receiver not influenced by target field or tau
            U_out_comps3(1) = greens(d_tgt_rec3, w(WW));
            U_out_comps3(2) = -1 * greens(d_tgt_recMirror3, w(WW));
            U_out3(XX,YY,WW) = sum(U_out_comps3);
            
            % Field from target to receiver
            U_s_comps(1) = U_out(XX,YY,WW)*tau*U_tgt(WW);
            U_s_comps(2) = U_out2(XX,YY,WW)*tau2*U_tgt2(WW);
            U_s_comps(3) = U_out3(XX,YY,WW)*tau3*U_tgt3(WW);
            U_s(XX,YY,WW)= sum(U_s_comps);
        ...
        for WW = 1:length(wSpec)
            ...
            U_inc_comps2(1) = greens(d_trans_tgt2, w(WW));
            U_inc_comps2(2) = -1 * greens(d_trans_tgtMirror2, w(WW));
            U_incSpec2(WW) = sum(U_inc_comps2);
            
            % Incident field transmitter to target3
            U_inc_comps3(1) = greens(d_trans_tgt3, w(WW));
            U_inc_comps3(2) = -1 * greens(d_trans_tgtMirror3, w(WW));
            U_incSpec3(WW) = sum(U_inc_comps3);
            
            % Total field at target
            U_tgtSpec(WW)=U_incSpec(WW)/(1-tau*(1/(4*pi))*(exp(i*w(WW)/c*2*tgtY))/(2*tgtY));
            
            % Total field at target2
            U_tgtSpec2(WW)=U_incSpec2(WW)/(1-tau2*(1/(4*pi))*(exp(i*w(WW)/c*2*tgt2Y))/(2*tgt2Y));
            
            % Total field at target3
            U_tgtSpec3(WW)=U_incSpec3(WW)/(1-tau3*(1/(4*pi))*(exp(i*w(WW)/c*2*tgt3Y))/(2*tgt3Y));
            ...
            U_out_comps2(1) = greens(d_tgt_rec2, w(WW));
            U_out_comps2(2) = -1 * greens(d_tgt_recMirror2, w(WW));
            U_outSpec2(XX,YY,WW) = sum(U_out_comps2);
            
            % Field at receiver not influenced by target field or tau
            U_out_comps3(1) = greens(d_tgt_rec3, w(WW));
            U_out_comps3(2) = -1 * greens(d_tgt_recMirror3, w(WW));
            U_outSpec3(XX,YY,WW) = sum(U_out_comps3);
            
            % Field from target to receiver
            U_sSpec_comps(1) = U_outSpec(XX,YY,WW)*tau*U_tgtSpec(WW);
            U_sSpec_comps(2) = U_outSpec2(XX,YY,WW)*tau2*U_tgtSpec2(WW);
            U_sSpec_comps(3) = U_outSpec3(XX,YY,WW)*tau3*U_tgtSpec3(WW);
            U_sSpec(XX,YY,WW)= sum(U_sSpec_comps);
