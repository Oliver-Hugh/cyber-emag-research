...
%        |   R   R   Xt target (fixed)   R   R   R   R   R   R   |
%        |                                                       |
%        P   R   R   R   R   R   R   R   R   R   R   R   R   R   |
%        E                                                       P
%        C   R   R   R   R   R   R   R   R   R   R   R   R   R   E
%        |                                                       C
%        |   R   R   R   R   R   R   R   R   R   R   R   R   R   |
% Mirror3|                                                       |
%        |   R   R   R   R   R   R   R   R   R   R   R   R   R   |
%        |        t transmitter (fixed)                          | Mirror 5
%        |   R   R   R   R   R   R   R   R   R   R   R   R   R   |
%        |                                                       |
% -------------PEC plate --------------------------------------------------
% Mirror2|   Mirror1                                             | Mirror4
...
for XX = 1:length(recX)
    for YY = 1:length(recY)
        ...
        d_trans_recMirror(4) = sqrt(((transX+2*d_trans_PEC3)-recX(XX))^2 + (transY+recY(YY))^2);
        d_trans_recMirror(5) = sqrt(((transX+2*d_trans_PEC3)-recX(XX))^2 + (transY-recY(YY))^2);
        ...
        d_trans_tgtMirror(4) = sqrt(((transX+2*d_trans_PEC3)+tgtX)^2 + (transY+tgtY)^2);
        d_trans_tgtMirror(5) = sqrt(((transX+2*d_trans_PEC3)+tgtX)^2 + (transY-tgtY)^2);
        ...
        d_tgt_recMirror(4) = sqrt(((tgtX+2*d_tgt_PEC3)+recX(XX))^2 + (tgtY+recY(XX))^2);
        d_tgt_recMirror(5) = sqrt(((tgtX+2*d_tgt_PEC3)+recX(XX))^2 + (tgtY-recY(XX))^2);
        
        %target self reflection distances
        d_tgt_mirror(1) = 2*tgtY;
        d_tgt_mirror(2) = 2*sqrt(tgtX^2 + tgtY^2);
        d_tgt_mirror(3) = 2*tgtX;
        d_tgt_mirror(4) = 2*sqrt(d_tgt_PEC3^2 + tgtY^2);
        d_tgt_mirror(5) = 2*d_tgt_PEC3;
...
%             U_bknd_comps(2) = 1/(4*pi)*1/d_trans_recMirror(1)*exp(i*w(WW)/c*d_trans_recMirror(1));
%             U_bknd_comps(3) = -1/(4*pi)*1/d_trans_recMirror(2)*exp(i*w(WW)/c*d_trans_recMirror(2));
%             U_bknd_comps(4) = 1/(4*pi)*1/d_trans_recMirror(3)*exp(i*w(WW)/c*d_trans_recMirror(3));
%             
%             U_bknd_comps(5) = -1/(4*pi)*1/d_trans_recMirror(4)*exp(i*w(WW)/c*d_trans_recMirror(4));
%             U_bknd_comps(6) = 1/(4*pi)*1/d_trans_recMirror(5)*exp(i*w(WW)/c*d_trans_recMirror(5));
            ...
            U_bknd_comps(5) = greens(d_trans_recMirror(4), w(WW));
            U_bknd_comps(6) = -1*greens(d_trans_recMirror(5), w(WW));
...
%             U_inc_comps(2) = 1/(4*pi)*1/d_trans_tgtMirror(1)*exp(i*w(WW)/c*d_trans_tgtMirror(1));
%             U_inc_comps(3) = -1/(4*pi)*1/d_trans_tgtMirror(2)*exp(i*w(WW)/c*d_trans_tgtMirror(2));
%             U_inc_comps(4) = 1/(4*pi)*1/d_trans_tgtMirror(3)*exp(i*w(WW)/c*d_trans_tgtMirror(3));
%             
%             U_inc_comps(5) = -1/(4*pi)*1/d_trans_tgtMirror(4)*exp(i*w(WW)/c*d_trans_tgtMirror(4));
%             U_inc_comps(6) = 1/(4*pi)*1/d_trans_tgtMirror(5)*exp(i*w(WW)/c*d_trans_tgtMirror(5));
            ...
            U_inc_comps(5) = greens(d_trans_tgtMirror(4), w(WW));
            U_inc_comps(6) = -1*greens(d_trans_tgtMirror(5), w(WW));
            
            U_inc(WW) = sum(U_inc_comps);
            
            
            % Target reflections
%             tgt_ref_comps(1) = -(tau*(1/(4*pi))*(exp(i*w(WW)/c*d_tgt_mirror(1)))/(d_tgt_mirror(1)));
%             tgt_ref_comps(2) = (tau*(1/(4*pi))*(exp(i*w(WW)/c*d_tgt_mirror(2)))/(d_tgt_mirror(2)));
%             tgt_ref_comps(3) = -(tau*(1/(4*pi))*(exp(i*w(WW)/c*d_tgt_mirror(3)))/(d_tgt_mirror(3)));
%             
%             tgt_ref_comps(4) = (tau*(1/(4*pi))*(exp(i*w(WW)/c*d_tgt_mirror(4)))/(d_tgt_mirror(4)));
%             tgt_ref_comps(5) = -(tau*(1/(4*pi))*(exp(i*w(WW)/c*d_tgt_mirror(5)))/(d_tgt_mirror(5)));
            ...
            tgt_ref_comps(2) = -1 * tgt_ref(d_tgt_mirror(4), w(WW));
            tgt_ref_comps(3) = tgt_ref(d_tgt_mirror(5), w(WW));
...
%             U_out_comps(2) = 1/(4*pi)*1/d_tgt_recMirror(1)*exp(i*w(WW)/c*d_tgt_recMirror(1));
%             U_out_comps(3) = -1/(4*pi)*1/d_tgt_recMirror(2)*exp(i*w(WW)/c*d_tgt_recMirror(2));
%             U_out_comps(4) = 1/(4*pi)*1/d_tgt_recMirror(3)*exp(i*w(WW)/c*d_tgt_recMirror(3));
%             
%             U_out_comps(5) = -1/(4*pi)*1/d_tgt_recMirror(4)*exp(i*w(WW)/c*d_tgt_recMirror(4));
%             U_out_comps(6) = 1/(4*pi)*1/d_tgt_recMirror(5)*exp(i*w(WW)/c*d_tgt_recMirror(5));
            ...
            U_out_comps(3) = greens(d_tgt_recMirror(4), w(WW));
            U_out_comps(4) = -1 * greens(d_tgt_recMirror(5), w(WW));
